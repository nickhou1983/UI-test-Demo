...
export default function TripEditPage() {
  ...
  if (!trip) {
    return (
      <main className="bg-sky-50 min-h-screen flex items-center justify-center">
        <div className="text-center">
          <p className="text-2xl text-slate-400 mb-4">{t('detail.notFound')}</p>
          <Link to="/trips" className="bg-blue-700 text-white px-6 py-2 rounded-full hover:bg-blue-800 transition">
            {t('trip.backToList')}
          </Link>
        </div>
      </main>
      ...
      <section className="bg-blue-800 text-white py-8">
        <div className="max-w-6xl mx-auto px-4">
          ...
          <input
            ...
            className="block text-2xl md:text-3xl font-bold bg-transparent border-b border-blue-600 focus:border-emerald-400 outline-none w-full max-w-lg pb-1"
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-lg text-sm font-medium transition ${
                    activeDay === day.dayNumber
                      ...
                      : 'bg-white text-slate-600 hover:bg-blue-50 shadow-sm'
                  }`}
                >
            ...
            <div className="bg-white rounded-xl shadow-md p-6">
              ...
              {currentDay && currentDay.activities.length > 0 && (
                <div className="space-y-3 mb-4">
                  {currentDay.activities.map((activity, idx) => (
                    <div
                      key={activity.id}
                      className="flex items-start gap-3 p-3 bg-sky-50 rounded-lg group"
                    >
                      <span className="w-6 h-6 bg-blue-700 text-white text-xs rounded-full flex items-center justify-center flex-shrink-0 mt-0.5">
                        {idx + 1}
                      </span>
              ...
              {showAddForm ? (
                ...
                <button
                  ...
                  className="w-full border-2 border-dashed border-slate-300 hover:border-blue-400 text-slate-400 hover:text-blue-500 py-3 rounded-lg text-sm transition"
                >
                  + {t('trip.addActivity')}
                </button>
              )}
          ...
          <div className="space-y-6">
            ...
            {dest && (
              <div className="bg-white rounded-xl shadow-md overflow-hidden">
                ...
                <div className="p-4">
                  <h3 className="font-bold text-blue-800">{t(dest.nameKey)}</h3>
