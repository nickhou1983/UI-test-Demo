  ...
  return (
    <main>
      ...
      <section className="max-w-7xl mx-auto px-4 py-16 text-center">
        <h2 className="text-3xl font-bold text-blue-600 mb-6">{t('about.mission')}</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {values.map((v) => (
            <div key={v.titleKey} className="bg-white rounded-xl shadow-md p-8">
              ...
              <h3 className="font-bold text-blue-600 text-lg mb-2">{t(v.titleKey)}</h3>
      ...
      <section className="bg-white py-16">
        <div className="max-w-7xl mx-auto px-4">
          <h2 className="text-3xl font-bold text-blue-600 text-center mb-10">{t('about.team')}</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {teamMembers.map((m) => (
              <div key={m.nameKey} className="text-center">
                <div className="w-32 h-32 rounded-full overflow-hidden mx-auto mb-4 bg-gradient-to-br from-blue-500 to-cyan-400 flex items-center justify-center">
                  <span className="text-4xl text-white">👤</span>
                </div>
                <h3 className="font-bold text-blue-600">{t(m.nameKey)}</h3>
                <p className="text-cyan-600 text-sm">{t(m.titleKey)}</p>
      ...
      {/* Contact */}
      <section className="max-w-7xl mx-auto px-4 py-16 text-center">
        <h2 className="text-3xl font-bold text-blue-600 mb-4">{t('about.contact')}</h2>
        <p className="text-slate-600 mb-4">{t('about.contact.text')}</p>
        <a href={`mailto:${t('about.email')}`} className="text-blue-500 hover:text-blue-700 font-medium text-lg">
          {t('about.email')}
        </a>
      </section>
    </main>
