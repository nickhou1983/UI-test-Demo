    ...
    return (
      <main className="bg-sky-50 min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <Link to="/destinations" className="bg-blue-700 text-white px-6 py-2 rounded-full hover:bg-blue-800 transition">
            {t('detail.backToList')}
          </Link>
        </div>
      </main>
    ...
    <main className="bg-sky-50 min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 py-4">
        <nav className="text-sm text-slate-500">
          <Link to="/" className="hover:text-blue-600">{t('detail.breadcrumb.home')}</Link>
          <span className="mx-2">/</span>
          <Link to="/destinations" className="hover:text-blue-600">{t('detail.breadcrumb.destinations')}</Link>
          <span className="mx-2">/</span>
          <span className="text-blue-800 font-medium">{t(detail.nameKey)}</span>
        </nav>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 pb-16">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            ...
            {/* Attractions */}
            <h2 className="text-2xl font-bold text-blue-800 mb-6">{t('detail.attractions')}</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-10">
              {detail.attractions.map((attr) => (
                <div key={attr.nameKey} className="bg-white rounded-xl shadow-md overflow-hidden">
                  ...
                  <div className="p-4">
                    <h3 className="font-bold text-blue-800 mb-1">{t(attr.nameKey)}</h3>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-md p-6">
              <h3 className="font-bold text-blue-800 text-lg mb-4">{t('detail.overview')}</h3>
        ...
        {relatedDests.length > 0 && (
          <section className="mt-16">
            <h2 className="text-2xl font-bold text-blue-800 mb-6">{t('detail.related')}</h2>
